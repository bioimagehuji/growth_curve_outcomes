...
class Convert:
    @staticmethod
    def hours_to_index(hours: float): return math.floor(hours * SAMPLES_PER_HOUR)
    @staticmethod
    def index_to_hours(index: int): return index / SAMPLES_PER_HOUR
    
    @staticmethod
    def mins_to_index(mins: int): return math.floor(mins / MINUTES_PER_SAMPLE)
    @staticmethod
    def index_to_mins(index: int): return index * MINUTES_PER_SAMPLE
...
class Repetition:
    ...
    def __post_init__(self):
        ...
        self.derivative_rolling_average = np.convolve(self.derivative, np.ones(ROLLING_WINDOW) / ROLLING_WINDOW, mode='valid')
    ...
    def plot(self):
        try:
            ...
            plt.plot(Convert.index_to_mins(self.lag_index), self.values[self.lag_index], marker='o', markeredgecolor="green", markerfacecolor="green")
            plt.plot(Convert.index_to_mins(self.max_rate_index), self.values[self.max_rate_index], marker='o', markeredgecolor="purple", markerfacecolor="purple")
...
def main():
    columns=('REP', 'GROUP', 'TOTAL_TIME', 'LAG_MINS', 'MAX_RATE_MINS', 'VALUE_AT_MAX_RATE', 'MAX_RATE', *(f'AUC_{hours}H' for hours in INTEGRAL_HOURS))
    data = []
